    ...
    override fun apply(target: Project) {
        with(target) {
            ...
            extensions.configure<ApplicationExtension> {
                ...
                signingConfigs {
                    // A project-local debug keystore, committed with the standard debug credentials,
                    // instead of the per-machine ~/.android/debug.keystore AGP would generate. It
                    // gives this laptop, any contributor and CI the same debug SHA-1, which is what
                    // the Firebase Android API key restriction is pinned to. The release config is
                    // created in the signing item, and only when keystore.properties exists.
                    getByName("debug") {
                ...
                buildTypes {
                    ...
                    release {
                        // R8 through the AGP 9 DSL; `isMinifyEnabled` is the legacy spelling and
                        // setting both is what confuses the baseline-profile plugin later.
                        optimization {
                            enable = true
                        }
                        proguardFiles(
